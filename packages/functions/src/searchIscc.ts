...
export const searchIscc = ApiHandler(async (_evt) => {
  console.log("searchIscc called");
  if (!_evt.headers["x-api-key"] || _evt.headers["x-api-key"] !== Config.SECRET_ZUPLO_ACCESS_KEY_SEARCH_B2B) {
    return {
      statusCode: 401,
      body: "Unauthorized",
    };
  ...
  const filteredUnderThreshold = vectorResults.filter((v) => {
    console.log(v);
    return v.score < 16;
  });
  ...
  if (filteredUnderThreshold.length === 0) {
    ...
    console.log("will return searchIscc:", willReturn);
    return willReturn;
  }
  ...
  // const areRedactedMap = await getIsRedactedByDeclarationIds(
  //   deduplicateArrayByField(
  //     sortedByScoreAscending.map((r: IS3PathMap) => r.docBody.metaInternal.declarationId || "").filter((i) => !!i)
  //   )
  // );
  ...
  const CENSORED = sortedByScoreAscending
    .filter(
      (r: IS3PathMap) => new Date(r.docBody.declarationMetadata.publicMetadata.timestamp) > new Date("2024-06-07"),
    ) //june 7th 2024 is when we added declarationId
